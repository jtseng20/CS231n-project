...
def crop_bbox_batch(feats, bbox, bbox_to_feats, HH, WW=None, backend='cudnn', style_batch=None):
  """
  Inputs:
  ...
  """
  if backend == 'cudnn':
    return crop_bbox_batch_cudnn(feats, bbox, bbox_to_feats, HH, WW, style_batch=style_batch)
  N, C, H, W = feats.size()
  B = bbox.size(0)
  if WW is None: WW = HH
  dtype, device = feats.dtype, feats.device
  crops = torch.zeros(B, C, HH, WW, dtype=dtype, device=device)
  style_batch_B = torch.LongTensor(B)
  for i in range(N):
    ...
    if style_batch is not None:
        style_batch_B[idx] = style_batch[i]
  return crops, style_batch_B
...
def crop_bbox_batch_cudnn(feats, bbox, bbox_to_feats, HH, WW=None, style_batch=None):
  N, C, H, W = feats.size()
  B = bbox.size(0)
  if WW is None: WW = HH
  ...
  if (all_idx == eye).all():
    return crops, style_batch_B
  return crops[_invperm(all_idx)], style_batch_B
