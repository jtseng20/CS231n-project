...
def train(args):
    ...
    for i in range(style_num):
        path = os.path.join(args.style_image, style_image[i])
        style = utils.load_image(path, size=args.style_size)
        ...
        for batch_id, x in enumerate(train_loader):
            ...
            if (batch_id + 1) % args.log_interval == 0:
                mesg = "{}\tEpoch {}:\t[{}/{}]\tcontent: {:.6f}\tstyle: {:.6f}\ttotal: {:.6f}".format(
                    time.ctime(), e + 1, count, len(train_dset),
...
def main():
    ...
    train_arg_parser.add_argument("--dataset", type=str, default="/vision2/u/helenav/datasets/vg/images",
                                  help="path to training dataset, the path should point to a folder "
                                       "containing another folder with all the training images")
    train_arg_parser.add_argument('--train_h5', default=os.path.join('/scr/helenav/datasets/preprocess_vg', 'train.h5'))
    train_arg_parser.add_argument("--style-image", type=str, default="/vision2/u/helenav/datasets/style-images/",
                                  help="path to style-image")
    train_arg_parser.add_argument("--save-model-dir", type=str, default='/scr/helenav/checkpoints_style_transfer',
                                  help="path to folder where trained model will be saved.")
    train_arg_parser.add_argument("--checkpoint-model-dir", type=str, default='/scr/helenav/checkpoints_style_transfer',
    ...
    train_arg_parser.add_argument('--num_train_samples', default=None, type=int)
...
class VgSceneGraphDataset(Dataset):
  def __init__(self, h5_path, image_dir, image_size=256, transform=None, max_samples=None):
